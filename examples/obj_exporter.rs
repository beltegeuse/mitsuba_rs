...
fn export_obj(scene_info: mitsuba_rs::Scene, wk: &Path) {
    ...
    for (i, shape) in shapes.into_iter().enumerate() {
        match shape {
            mitsuba_rs::Shape::Obj{
                ...
                panic!("Obj file: {:?}", filename);
                // println!("Obj file: {:?}", filename);
            ...
            mitsuba_rs::Shape::Ply { filename, option, .. } => {
                ...
                let filename_obj = filename.split('.').next().unwrap();
                let filename_obj = format!("{}.obj", filename_obj);
                let filename_obj = Path::new(&filename_obj);
                let filename_obj = wk.join(filename_obj);

                let file = File::create(&&filename_obj).unwrap();
                ...
                let filename = wk.join(filename);
                println!("Convert {} => {}", filename.to_str().unwrap(), filename_obj.to_str().unwrap());
